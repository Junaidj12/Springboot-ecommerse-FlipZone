...
public class CustomerController {
	...
	@GetMapping("/home")
	public String loadCustomerHome(HttpSession session, ModelMap model, @RequestParam(defaultValue = "") String name,
			@RequestParam(defaultValue = "default") String sort, @RequestParam(defaultValue = "all") String stock) {
		if (session.getAttribute("customer") == null) {
			session.setAttribute("fail", "Invalid session. Please login again");
			return "redirect:/login";
		}

		return customerService.loadProductsForCustomer(session, model, name, sort, stock);
	}
